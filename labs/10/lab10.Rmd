...
```{r plot-p2nn-db, cache=T, fig.width=9, fig.height=9, eval=F}
granularity <- 1000
...
cat("Epochs requested:", 500, fill=T)
...
fit_convnet <- function(x, y, epochs=500, val=0, early_stop=T) {
    ...
    if (early_stop) callback <- callback_early_stopping(monitor="loss", patience=5)
    else callback <- NULL
...
train_y <- to_categorical(subset(train, select=digit))

test_x <- as.matrix(subset(test, select=-digit))
test_xconv <- array_reshape(test_x, c(nrow(test_x), 16, 16))
test_y <- to_categorical(subset(test, select=digit))
...
- In this report, the keras package is used to explore the application of neural networks to the zipcode handwritten digits dataset. The data is first visualised with a random sample of each digit's training observations. Using only the two predictors known to be most informative, networks with varying numbers of hidden nodes are fit, and results plotted. Then, a two-hidden-layer network is fit, both with validation holdout and without. The same network is then trained with all 256 predictors, and compared with a convolutional network trained in the same way. Finally, a convolutional network is trained on all 10 digits and performance measured.
