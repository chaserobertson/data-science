...
```{r costs-plot, cache=T, fig.height=4, fig.width=4}
costs <- 10^(-4:-1)
form <- digit ~ p105 + p167
r <- lapply(costs, function(x) svm(form, data=predictors, kernel='linear', cost=x))
...
- Training and test errors should indeed be very similar for each of these linear models. Linear models are not flexible enough to make overfitting very likely.
...
# CV not necessary
#set.seed(1234)
#rt <- tune(svm, form, data=predictors, validation.x=test[,2:3], validation.y=test[,1], kernel='linear', ranges=list(cost=10^(-4:4)))
#c(rt$best.parameters, rt$best.performance)
```
...
```{r gamma-fit, cache=T}
gamma <- 10^c(-4, -1, 1, 3)
r <- lapply(gamma, function(x) svm(form, data=predictors, kernel='radial', cost=best_cost, gamma=x, probability=T))
```
...
```{r gamma-plot, cache=T, fig.width=4, fig.height=4}
plot(p167, p105, col=col, main="Radial")  # scatter plot
for (i in seq_along(gamma)) {
    fit <- r[[i]] 
    probs <- attr(preds[[i]], "probabilities")[,1]
    z <- matrix(probs, nrow=length(p167))
    contour(x, y, z, levels=0.5, lwd=2, labels=paste0("g=", fit$gamma), 
            drawlabels=T, method="flattest", labcex=1.5, add=TRUE)
}
# add decision boundarys
```

What is the effect of gamma here, in terms of the decision boundary and the number of support vectors?

- An increase of gamma leads to a more flexible nonlinear boundary, with very small gamma fitting a straight line, and very high gamma fitting a wiggling line which fits the noisy training data very tightly.
...
- Given that the lowest test error was achieved with gamma=10, I think that using a radial kernel does help here, as compared to the linear kernel. It is crucial to tune gamma properly, though, as the other attempted values do not lead to superior performance over the linear kernel models.
...
```{r all-pred, cache=T}
detach(predictors)
r <- svm(digit ~ ., data=train, kernel='radial', cost=0.1, gamma=10)
...
```{r boston-svm, cache=T, fig.width=4, fig.height=4}
br <- svm(medv ~ lstat, data=Boston, kernel='radial', scale=F, 
          eps=0.05, cost=0.1, gamma=0.1)

plot(lstat, medv, main='Radial Regression')
lines(lstat, predict(br, Boston), lwd=2)
```
...
br <- tune(svm, medv ~ ., data=Boston, kernel='radial', scale=F, 
          ranges=list(eps=c(0.1, 0.05), cost=c(0.1, 1), gamma=c(0.1, 1)))
...
br <- tune(svm, medv ~ ., data=Boston, kernel='radial', scale=TRUE, 
          ranges=list(eps=c(0.1, 0.05), cost=c(0.1, 1), gamma=c(0.1, 1)))
